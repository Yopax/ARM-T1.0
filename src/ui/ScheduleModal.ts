...
import { BimCategory, BimElementDocument } from '../core/database/BimDatabaseTypes';
...
export class ScheduleModal {
  ...
  private render(): void {
    this.container.innerHTML = `
      <div class="bg-slate-900 border border-slate-700 w-full max-w-6xl max-h-[90vh] rounded-xl shadow-2xl flex flex-col overflow-hidden text-slate-200">
        <!-- HEADER -->
        <div class="px-5 py-3.5 border-b border-slate-800 flex items-center justify-between bg-slate-950/70">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span class="text-[10px] bg-sky-950 text-sky-400 border border-sky-800 px-2 py-0.5 rounded-full font-mono">Revit Schedule / MongoDB BSON</span>
              </h2>
              <p class="text-[11px] text-slate-400">Metodología Relacional Orientada a Objetos con sincronización bidireccional en tiempo real</p>
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button id="btn-sched-export-json" class="px-2.5 py-1 text-xs font-semibold bg-emerald-900/60 hover:bg-emerald-800 text-emerald-200 border border-emerald-700 rounded-md transition-colors flex items-center gap-1.5 cursor-pointer">
        ...
        <!-- FILTROS Y BÚSQUEDA -->
        <div class="px-5 py-3 border-b border-slate-800 bg-slate-900/50 flex flex-wrap items-center justify-between gap-3">
          <div class="flex flex-wrap items-center gap-3">
            <div class="flex items-center gap-1.5">
              <label class="text-[11px] font-semibold text-slate-400 uppercase">Categoría:</label>
              <select id="sched-filter-category" class="bg-slate-950 border border-slate-700 rounded px-2.5 py-1 text-xs text-slate-200 outline-none focus:border-sky-500">
            ...
            <div class="flex items-center gap-1.5">
              ...
              <select id="sched-filter-sector" class="bg-slate-950 border border-slate-700 rounded px-2.5 py-1 text-xs text-slate-200 outline-none focus:border-sky-500">
            ...
            <div class="flex items-center gap-1.5">
              ...
              <input id="sched-filter-search" type="text" placeholder="ID, GUID, Marca..." class="bg-slate-950 border border-slate-700 rounded px-2.5 py-1 text-xs text-slate-200 outline-none focus:border-sky-500 w-36" />
            </div>
          </div>

          <!-- TARJETAS DE RESUMEN Y MÉTRICAS -->
          <div id="sched-summary-cards" class="flex items-center gap-3 text-xs">
            <!-- Renderizado dinámico -->
          </div>
        </div>

        <!-- TABLA DE DATOS -->
        <div class="flex-1 overflow-auto p-0 min-h-75">
          <table class="w-full border-collapse text-left text-xs">
            <thead class="sticky top-0 bg-slate-950 text-slate-400 text-[11px] uppercase tracking-wider font-semibold border-b border-slate-800 z-10">
              <tr>
        ...
        <!-- FOOTER INFO -->
        <div class="px-5 py-2 border-t border-slate-800 bg-slate-950/70 flex items-center justify-between text-[11px] text-slate-400">
          <span class="flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            Base de datos relacional bidireccional conectada a memoria y preparada para MongoDB.
          </span>
          <span>Tip: Al editar la Marca o Sector en la tabla, el modelo 3D y la paleta de propiedades se sincronizan de inmediato.</span>
        </div>
      </div>
    `;
  ...
  private renderTable(): void {
    ...
    const { records, summary } = db.querySchedule({
    ...
    if (summaryCards) {
      summaryCards.innerHTML = `
        <div class="px-2.5 py-1 bg-slate-800 rounded border border-slate-700">
      ...
      .map((r) => {
        ...
        return `
          <tr class="hover:bg-slate-800/40 transition-colors text-slate-300" data-guid="${r.uniqueId}">
            ...
            <td class="py-2 px-3 text-right text-purple-400">$${r.instanceParameters.estimatedCost.toFixed(2)}</td>
            <td class="py-2 px-3 text-center">
              <button class="btn-focus-element px-2 py-0.5 bg-sky-950 hover:bg-sky-900 text-sky-400 border border-sky-800 rounded font-sans text-[10px] font-semibold cursor-pointer transition-colors" data-guid="${r.uniqueId}" data-element-id="${r.elementId}">
                🎯 Ver 3D
              </button>
            </td>
          </tr>
  ...
  private bindTableInteractions(): void {
    ...
    // Seleccionar y enfocar en el modelo 3D
    this.container.querySelectorAll<HTMLButtonElement>('.btn-focus-element').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (elementId || guid) {
          this.close();
          this.onSelectElement(elementId ? parseInt(elementId, 10) : guid!);
        }
      });
    });
  }
