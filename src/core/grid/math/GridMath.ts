...
export class GridMath {
   ...
   */
  public static isVertical(grid: GridElement, tolerance = 0.2): boolean {
   ...
   */
  public static isHorizontal(grid: GridElement, tolerance = 0.2): boolean {
    if (grid.geomType !== 'line') return false;
  ...
  public static applyDragUpdate(
    ...
    const isVert = this.isVertical(mainGrid, 0.2);
    const isHoriz = this.isHorizontal(mainGrid, 0.2);
  ...
  public static computeElbowGeometry(grid: GridElement): {
    ...
    const breakLen = Math.min(safeLen * 0.4, 4.0);

    // 1. Extremo inicial
    const isStartElbow = !!grid.startElbow?.active;
    const startOffset = grid.startElbow?.lateralOffset || -2.5;
    ...
    if (isStartElbow) {
      ...
      startBubblePos = { x: k3.x - ux * 1.8, z: k3.z - uz * 1.8 };
      startKneePos = k2;
      startElbowGripPos = k2;
      startIconPos = { x: k1.x, z: k1.z };
    } else {
      startPolyPoints.push({ x: grid.start.x, z: grid.start.z });
      startBubblePos = { x: grid.start.x - ux * 1.8, z: grid.start.z - uz * 1.8 };
      startIconPos = { x: grid.start.x + ux * 2.2, z: grid.start.z + uz * 2.2 };
    }

    // 2. Extremo final
    const isEndElbow = !!grid.endElbow?.active;
    const endOffset = grid.endElbow?.lateralOffset || -2.5;
    ...
    if (isEndElbow) {
      ...
      endBubblePos = { x: k3.x + ux * 1.8, z: k3.z + uz * 1.8 };
      endKneePos = k2;
      endElbowGripPos = k2;
      endIconPos = { x: k1.x, z: k1.z };
    } else {
      endPolyPoints.push({ x: grid.end.x, z: grid.end.z });
      endBubblePos = { x: grid.end.x + ux * 1.8, z: grid.end.z + uz * 1.8 };
      endIconPos = { x: grid.end.x - ux * 2.2, z: grid.end.z - uz * 2.2 };
    }
  ...
  public static calculateElbowLateralOffset(
    ...
    if (Math.abs(clamped) < 0.4) {
      return clamped < 0 ? -0.5 : 0.5;
    }
