...
export class BimDatabase {
  ...
  public updateTypeParameters(
    ...
    for (const doc of this.elements.values()) {
      if (doc.typeId === typeId) {
        Object.assign(doc.typeParameters, updates);
        if (updates.typeName) {
          doc.familyType = updates.typeName;
        }
        if (updates.unitCost !== undefined) {
          doc.instanceParameters.estimatedCost = Number(
            (doc.instanceParameters.volume * updates.unitCost).toFixed(2)
          );
        }
        if (updates.concreteStrength !== undefined) {
          doc.instanceParameters.concreteStrength = updates.concreteStrength;
        }
  ...
  public changeElementType(guid: string, newTypeId: string): BimElementDocument | undefined {
    ...
    // Recalcular volumen según el nuevo tipo
    if (doc.category === 'OST_StructuralColumns' && newType.width && newType.depth) {
      const h = doc.instanceParameters.height || 3.5;
      const vol = newType.width * newType.depth * h;
      doc.instanceParameters.volume = Number(vol.toFixed(3));
      doc.instanceParameters.surfaceArea = Number((2 * (newType.width + newType.depth) * h).toFixed(2));
      doc.geometry.dimensionsString = `${newType.width}m × ${newType.depth}m (h=${h.toFixed(2)}m)`;
    } else if (doc.category === 'OST_StructuralFraming' && newType.width && newType.height) {
      const l = doc.instanceParameters.length || 6.0;
      const vol = newType.width * newType.height * l;
      doc.instanceParameters.volume = Number(vol.toFixed(3));
      doc.instanceParameters.surfaceArea = Number(((newType.width + 2 * newType.height) * l).toFixed(2));
      doc.geometry.dimensionsString = `${newType.width}m × ${newType.height}m (L=${l.toFixed(2)}m)`;
    } else if (doc.category === 'OST_StructuralFoundation' && newType.width && newType.depth && newType.height) {
      const vol = newType.width * newType.depth * newType.height;
      doc.instanceParameters.volume = Number(vol.toFixed(3));
      doc.instanceParameters.surfaceArea = Number((4 * newType.width * newType.height).toFixed(2));
      doc.geometry.dimensionsString = `${newType.width}m × ${newType.depth}m × ${newType.height}m`;
    }

    doc.instanceParameters.concreteStrength = newType.concreteStrength;
    doc.instanceParameters.estimatedCost = Number(
      (doc.instanceParameters.volume * newType.unitCost).toFixed(2)
    );
