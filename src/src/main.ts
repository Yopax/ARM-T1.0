...
async function bootstrap() {
  ...
  contextualBar = new ContextualSubheader({
    ...
    onStartSketchMode: () => {
      const el = selection.selectedElement;
      if (el && el.type === 'slab') {
        const def = gripManager.ensureDefinition(el) as SlabDefinition;
        const p0 = def.boundary[0];
        const p1 = def.boundary[1];
        const newV: Vector3D = { x: (p0.x + p1.x) / 2 + 1.2, y: def.elevationY, z: (p0.z + p1.z) / 2 - 1.2 };
        def.boundary.splice(1, 0, newV);
        const meshData = wasm.createPolygonSlab(def.boundary, def.voids, def.thickness, def.elevationY);
        el.mesh.geometry.dispose();
        el.mesh.geometry = meshData.geometry;
        el.line.geometry.dispose();
        el.line.geometry = new THREE.EdgesGeometry(meshData.geometry, 20);
        el.volume = meshData.volume;
        el.dimensions = `Losa Poligonal N=${def.boundary.length} (e=${def.thickness.toFixed(2)}m)`;
        gripManager.updateGripsForElement(el);
        selection.refreshHighlight();
        sidebar.showElementProperties(el);
        footer.setMessage(`Boceto de losa: Vértice libre N=${def.boundary.length} añadido. Arrastra los grips para dar forma.`);
        structural.updateMetrics();
      }
    },
    onAddVoidSketch: () => {
      ...
      if (el && el.type === 'slab') {
        const def = gripManager.ensureDefinition(el) as SlabDefinition;
        if (!def.voids) def.voids = [];
        const cx = def.boundary.reduce((acc, p) => acc + p.x, 0) / def.boundary.length;
        const cz = def.boundary.reduce((acc, p) => acc + p.z, 0) / def.boundary.length;
        const hole: Vector3D[] = [
          { x: cx - 0.75, y: def.elevationY, z: cz - 0.75 },
          { x: cx + 0.75, y: def.elevationY, z: cz - 0.75 },
          { x: cx + 0.75, y: def.elevationY, z: cz + 0.75 },
          { x: cx - 0.75, y: def.elevationY, z: cz + 0.75 },
        ];
        ...
        el.line.geometry = new THREE.EdgesGeometry(meshData.geometry, 20);
        el.volume = meshData.volume;
        el.dimensions = `Losa con Shaft Central - e=${def.thickness.toFixed(2)}m`;
        gripManager.updateGripsForElement(el);
        selection.refreshHighlight();
        sidebar.showElementProperties(el);
        footer.setMessage('Perforación interior (Shaft Opening) creada en la losa. Arrastra los grips naranjas.');
        structural.updateMetrics();
      }
    },
  });
